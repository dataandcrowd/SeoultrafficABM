...
library(tidyquant)
setwd("D:/github/seoulbigdata")
admin <- read.csv("GIS/admin.csv", header = T) #%>% as.data.frame()

Adminpop$Date <- as.Date(as.character(Adminpop$date), format = "%Y%m%d",tz = "Asia/Seoul")
popclean <- Adminpop %>% 
            select(-c(H_SDNG_CD,DO_NM,CT_NM,H_DNG_NM)) %>% 
            reshape2::melt(id = c("admincd", "Date", "date", "hour"),
                           variable.name = "pop_type", value.name = "pop_value")
totalpop <- filter(popclean, pop_type == "totalpop", Date == "2018-04-10", admincd %in% admin[[2]]) %>% 
            left_join(admin, by = c("admincd"="hjb_code")) %>% 
            select(-pop_type) %>% arrange(admincd, date, hour) 
totalpop$pop_value <- round(totalpop$pop_value,0) #Round decimal values
