...
;    ask individual [
;      let o [origin] of self
;      let d [destination] of self
;      ;let od (patch-set o d)
;      if not any? o or not any? d [ set-path shortest-path ]
;     ]
...
to move
  ask people [
    ...
    let target-patch min-one-of neighbors4 with [navigated? = true and
      step > [step] of myself] [total-cost]
    let target-patch2 min-one-of neighbors4 with [navigated? = true] [total-cost]
