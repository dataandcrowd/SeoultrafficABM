...
#########################
hl <- read_feather("CBD_HL0_4486.feather") %>% clean_names()
...
#  ylim(0,7) +
  labs(title = "Sensitivity Analysis: Assessing Subway Commuters Health at Risk - Zero Cars",
...
ggsave("Health_Zero_e.png", plot_commuters, width = 7, height = 7, dpi = 300, type = "cairo")
...
hl %>% 
  ...
  geom_line(aes(colour = health_loss)) +
  #  geom_ribbon(aes(ymin = drivers_p - drivers_p_sd, ymax = drivers_p + drivers_p_sd),alpha=0.5) +
  facet_grid(health_loss ~.) +
  ylim(0,10) +
  labs(title = "Sensitivity Analysis: Assessing Drivers Health at Risk - Zero Cars",
       subtitle = "Health Loss Parameters: .03 .05 .01 .15 .2",
       x = "",
       y = "Population at risk (%)",
...
ggsave("Health_Zero_d.png", plot_drivers, width = 7, height = 7, dpi = 300, type = "cairo")
...
ggsave("Individual_health_zero.png", health, width = 7, height = 7, dpi = 300, type = "cairo")
