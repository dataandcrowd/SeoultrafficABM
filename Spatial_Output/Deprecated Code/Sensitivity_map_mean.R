emission <- read_feather("CBD_Emission.feather") %>% clean_names() %>% 
...
emission_long %>%
  ...
  geom_hline(yintercept=50, linetype="dashed", alpha = .5) +
  ...
  theme(legend.position = "none",
        text = element_text(size=11)) -> plot_emission_samil_mean
...
emission_long %>%
  ...
  theme_bw() +
  theme(legend.position = "none",
        text = element_text(size=11)) -> plot_emission_jongno_mean
