...
to setup-people
  ...
  [
    ...
    set color black
    if [pcolor] of patch-here = brown + 3 [move-to one-of patches with [pcolor != brown + 3]]
...
ask people [
  ...
  let allroads [self] of patches with [pcolor = white]
  let d one-of patches with [pcolor = green]


  foreach allroads [ road_patch ->
    ask road_patch [set Heuristic distance d]
    ]
end
    ...
    [ set pcolor yellow
      let laststep [step] of one-of neighbors4 with [pcolor = yellow or
        ...
        ifelse [plabel] of patches != nobody [
          ...
          set plabel-color black + 1][set plabel ""]
  ]]]
end
...
to shortest-path
  ...
  [ let check-steps min-one-of neighbors4 with [pcolor != brown + 3 and pcolor != white][step]
  ...
  ask patches with [pcolor = green] [
    ...
    if nav-patch != nobody [ask nav-patch [set pcolor green set paths fput self paths]]
    ]]
  ask person 0 [set path paths]
    ]
...
to move
  ...
  ;if all? [destination] of people  [pcolor = green][stop]

  ask people [
    let tt [destination] of self
    let target-patch min-one-of neighbors4 with [pcolor = green and
      step > [step] of myself] [total-cost]
    ;output-print target-patch

    let target-patch2 min-one-of neighbors4 with [pcolor = green ] [total-cost]
    ;print [step] of target-patch2

    ifelse distance destination <= 1
       [ move-to destination ]
    [ ifelse target-patch != nobody
       [ move-to target-patch ]
       [ move-to target-patch2 ]
  ]
  ...
  let temp []
  ask people [
    ask [neighbors4] of patch-here [
      if pcolor != brown + 3 [set temp lput self temp]
  ]]
end
@#$#@#$#@
GRAPHICS-WINDOW
